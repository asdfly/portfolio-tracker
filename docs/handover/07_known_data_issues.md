...
| 十一 · `portfolio_snapshots` 复制行 | 中 | 中 | `2026-06-15 ~ 06-29` 的 10 只场外标的为 fill-forward 复制行（基期实为 `06-12`），该段场外市值被系统性低估、`portfolio_summary.total_value` 同步偏低，并污染相关性 / 波动率。**既有三道守卫（跨期 >12 天 / 尖峰 >0.30 / 零方差）对本段无效** —— 复制行内部 `gap = 1` 从 12 天闸门下方通过，`06-30` 假跳变 `|log_ret| = 0.1698 < 0.30`（实测 `base` 档对这 4 根**一根都没拦下**）。2026-09-16 新增 **copy 规则（B2）**：**仅场外侧**（`is_otc_fund` 白名单，已核验 `166301`/`519770` 正确判为场外）、`(current_price, market_value 取到分)` 与上一行相同 ⇒ 该行非观测、基期回退到最近真值观测。实测：零收益观测 **107 → 0**、`average_correlation` 0.3480 → **0.4113**、`high_correlation_pairs` 76 → **102**。**但伪行与那根假跳变仍留在库里 —— 守卫只让它们进不了相关矩阵，不是根治**；且写入方**仍存活**（08-03 后 17 段，最近 `09-14→09-15` 命中全部 12 只场外）。已知代价：`100032` 三条单只同价观测被一并 void（疑为真实平价日），聚合影响 1e-4。 |
...
### 三、⚠️ 口径 regime 分界 = `2026-08-03`；**不要做 +1 日对齐**（已实测证伪）

`06-30` 那行恰好等于 `06-29` 的净值，极像「快照行全局滞后一天」。**已实测证伪：**

> 对 2026-08-03 之后的 7 只场外标的、各 20 个可比日，逐日与官方单位净值比对：
> **`== 净值(D)`：19 天 ｜ `== 净值(D-1)`：1 天 ｜ 两者都不等：0 天。**
>
> ⇒ **日频段是对齐的。**
> **不要对场外序列做 +1 日对齐** —— 那会把本来对齐的日频段整体推歪。
> 异常**只限 `2026-08-03` 之前**（旧写入路径：陈旧 / 滞后）。
> **本条为「已实测证伪」的结论，请勿重新怀疑并据此去修。**

即口径在 **`2026-08-03`** 分界：之前走旧写入路径（陈旧、复制行），
之后走 `scripts/fetch_otc_fund_nav.py`（精确对齐当日净值）。
...
> **上述判别标准 (1)/(2) 为当时提出的设计，最终落地的不是它们。**
> 本节保留原判据原文（含下方注记），因为那是当天的推理记录；**不要因为实现不同就把它们删掉**。

**最终落地的是 copy 规则（B2）**：

> **仅场外侧**（`is_otc_fund` 白名单）、`(current_price, market_value 取到分)` 与上一行相同
> ⇒ 该行判为非观测，**基期回退到最近真值观测**。

**B2 的已知代价与它为何是「疑为」**：`100032` 在 `06-15~06-29` 段内的恒定价 `0.9780`
相对官方 `06-15` 的偏离是 **+0.82%**，而其余 9 只**全部为负**（−0.35% ~ −8.60%）——
**方向不一致**说明它那段与其它 9 只可能不同源，故记为「疑为真实平价日」，**不强行归因**。

**与 (1) 的差异：不使用 `length >= 5` 段长阈值。** 原因（实测）：
...
**与 (2) 的差异：未实现横截面广度判据。** B2 的结果与另一条线用**完全不同的方法**
（特例签名）独立算出的 `0.4112 / 101` 只差 `1e-4` / 1 个标对 —— 为 `1e-4` 再加一层机制不值当。

**与 (4) 的关系：方向一致，B2 即 (4) 的落地形态。** (4) 要求「按有效基期算 gap」+「复制行内部收益记 `NaN`」；
B2 把复制行判为非观测并让基期回退到最近真值观测，实测零收益观测归零，即复制行内部收益
**已不再落成 `0.0` 混进矩阵**。区别仅在 gating 手段：B2 直接用 copy 规则 void 掉该行，
而 (4) 原设想是让 gap 变大后由跨期闸门接住 —— 结果等价，B2 少一次间接。

**实测效果（真实 34 只持仓副本）**：零收益观测 **107 → 0**；
`average_correlation` 0.3480 → **0.4113**；`high_correlation_pairs` 76 → **102**。

**适用范围风险（待办）**：B2 的作用域由 `is_otc_fund` 的白名单 `OTC_FUND_CODES` 决定 ——
**新增场外标的若未同步登记，copy 规则静默不生效**。
