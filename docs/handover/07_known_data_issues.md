...
> 若口径变更，vol / mdd / sharpe **必须重判**——尤其夏普的**符号**（见下）。
...
2. **08-03 之后只在 6 个周五有差**（占该段 30 个交易日的 20%）。而场外自 08-03 起
   已是日频（见上表）。若 B 真按日补齐场外，该段应当**每日**都有差。
   这个周期性说明 B 在该段仍按「观测日」注入，而不是逐日——**B 的构造尚待其作者核实**。

因此 **(a)「真实场外日频补齐」并非恒为 0，但仅 ≈ 1% 量级**（−0.1040pp / −10.0896pp），
在结论上可忽略；B 的 Δ 实质上就是 (c) 月末一次性记账。
...
**A 丢水平、保日序列；B 补水平、毁日序列。两者都不是「真」。**

#### 待评估的第三条路线 C'（区间摊销）

对「两端有快照、中间无观测」的标的，取区间总对数收益 `r = ln(mv_curr / mv_prev)`，
**均摊到区间内每个交易日**（每日 `r / N`）。效果：**水平与 B 一致，且不制造伪尖峰**
（把 B 那个单日 −8.66% 拆回它本应归属的 ~31 天里）。

它仍**不是真值**——无观测 ⇒ 月内路径不可知，且均摊会系统性**低估 vol**。
但在「水平 + 不造假尖峰」两个判据上同时优于 A 与 B。**评估中，尚未采纳。**
