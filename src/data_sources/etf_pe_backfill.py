...
覆盖说明（上游数据缺口，非实现问题）
--------------------------------------
16 个跟踪指数中 **13 个**可经 csindex 取到 2018→至今全量 PE（覆盖 20/23 ETF）。
以下 3 个 csindex 未发布 PE 历史，估值因子对这些 ETF 保持禁用，引擎自动跳过：

- `399673` 创业板50        → `159949` 创业板50ETF
- `930006` 中证机器人      → `159732` / `159770` 机器人ETF
- `h11118` 港股通高股息低波动(跨境) → `159220`（港股 ETF，无 A 股 PE，属正常）

如需补齐这 2 个 A 股指数（创业板50 / 中证机器人）的 PE，需改用 eastmoney /
wind 等付费源，本报告环境内 csindex + 乐咕均不可得。
"""
