...
事故（2026-09-16 15:30 日频管线，日志 logs/portfolio_20260916.log）::
...
下方一律用**日志原文内容锚**定位，**不写行号**：该日志是长跑追加文件，行号会漂
（同一批锚点在不同时点读取到的行号可以相差数千行），引用行号的注释会在下一次运行后
静默变成假陈述::

    「[001194] 最新净值 2026-09-16 | 待插入 0 行」                ← 13 只逐一同形
    「[otc_nav_missing] 场外当日净值缺失 12 只(目标日 2026-09-17)」
    「…这些标的本日**不落快照行**(禁止静默跳过)」                  ← 此句与事实不符
    「[持仓合并] 这 12 只取自 2026-09-16（价格沿用该日净值…）」
    「保存持仓快照: 2026-09-17, 34条记录」                         ← code 集合与 09-16 完全相同
    「[口径B] daily_return 排除 12 只价格非当日的标的…22/34 只，61.08%」
    「[EMAIL] [CRITICAL] 数据未就绪，拒绝生成/发送今日…日报：… run_status=degraded」
    「[WARN] report email send FAILED, rc=1」
    最终         总市值 1,524,395.24 / 当日收益 -0.38%，portfolio_summary 已正常落库
    未生成       data/reports/email_report_20260917_light.html

即：这 12 只**确实落了当日快照行**（「保存持仓快照: 2026-09-17, 34条记录」的 code 集合
与 09-16 相同），却被「[otc_nav_missing] …这些标的本日不落快照行」那句打进 degraded
⇒ `scripts/send_report_email.py` 的 `_RUN_STATUS_BLOCKING` 会**每一个交易日**都拒发
日报。真正代表 09-16 事故形态的契约 2（check_snapshot_baseline）当天反而是**通过**的
（告警里没有 summary_refused_snapshot_incomplete）。

⚠️ 时序前提（本闸门最容易踩的坑；2026-09-17 15:30 首跑实测）::

    `filled` 形态只能在同一日的 `portfolio_snapshots` 行**落库之后**才可观测。
    写当日快照的是**阶段一**：`run_analysis.run_stage1_basic` → `src/analysis/portfolio.py`
    的持仓合并路径，留痕为日志「保存持仓快照: <今天>, <N>条记录」。
    若把本检查排在阶段一**之前**（曾经如此），那一刻当日快照一行都没有 ⇒ 永远
    filled=0 / uncovered=全部 ⇒ alert_level='error' ⇒ 每个交易日都误报、日报被拦死。
    实测同一批锚点的时间戳：契约1 告警于 **15:30:43,930** 打出，而
    「保存持仓快照: 2026-09-17, 34条记录」于 **15:30:45,386** 才发生 —— 早 1.5 秒。
    该调用点位置由 `tests/test_snapshot_baseline_gate.py::
    test_contract1_check_must_run_after_snapshot_write` 守着（源码顺序断言）。
