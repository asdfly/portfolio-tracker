...
def _render_signal_overview(tech_df, n_bullish_ma, n_bearish_ma, n_overbought, n_oversold, n_strong_up, n_weak_down, n_total):
...
def _render_signal_charts(tech_df, n_bullish_ma, n_bearish_ma, n_bull_macd, n_bear_macd, n_bull_kdj, n_bear_kdj, n_overbought, n_oversold, n_strong_up, n_weak_down):
    st.markdown(
...
def _render_signal_table(tech_df):
    st.markdown(
...
def _render_bollinger_chart(tech_df, n_total):
    st.markdown(
...
def _render_rsi_chart(tech_df, n_total):
    # ---------- RSI 分布图 ----------
    st.markdown(
...
def render_tab6():
    ...
    if tech_df is None or tech_df.empty:
        ...
        n_strong_up = len(tech_df[tech_df["trend"] == "强势上涨"])
        n_weak_down = len(tech_df[tech_df["trend"].isin(["下跌", "温和下跌"])])
        _render_signal_overview(tech_df, n_bullish_ma, n_bearish_ma, n_overbought, n_oversold, n_strong_up, n_weak_down, n_total)
        _render_signal_charts(tech_df, n_bullish_ma, n_bearish_ma, n_bull_macd, n_bear_macd, n_bull_kdj, n_bear_kdj, n_overbought, n_oversold, n_strong_up, n_weak_down)
        _render_signal_table(tech_df)
        _render_bollinger_chart(tech_df, n_total)
        _render_rsi_chart(tech_df, n_total)
