...
def compute_rebalance_suggestion(target_weights=None, threshold=0.05):
    """compute_rebalance_suggestion（委托到 data_loader）"""
    import data_loader as _dl
    return _dl.compute_rebalance_suggestion(target_weights=target_weights, threshold=threshold)

def run_monte_carlo(days=252, n_simulations=500, end_date=None):
    """run_monte_carlo（委托到 data_loader）"""
    import data_loader as _dl
    return _dl.run_monte_carlo(days=days, n_simulations=n_simulations, end_date=end_date)
