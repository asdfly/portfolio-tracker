...
def _render_year_overview(cal_data):
    # --- 年份选择 ---
...
def _render_annual_trend(all_cal):
    ...
    if not all_cal.empty:
        yearly_cumret = all_cal.groupby("year", group_keys=False).apply(
...
def render_tab4():
    ...
    if cal_data.empty:
        ...
        sel_year, year_df, sel_month = _render_year_overview(cal_data)
        month_df = year_df[year_df["month"] == sel_month]
        _render_monthly_view(month_df, sel_year, sel_month)
        all_cal = load_calendar_data()
        _render_heatmap()
        _render_annual_trend(all_cal)
        _render_boxplot(all_cal)
