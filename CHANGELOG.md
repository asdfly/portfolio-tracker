...
### 2026-09-17 ~ 2026-09-22 后补登记（09-03 之后的变更此前未入册）

#### 修复
- **每日管线崩溃 (3d64265)**：恢复被死代码清理误删的 `_calculate_technical_indicators`（`fbe07ee` 误删 + 调用点漏改 ⇒ 阶段四 AttributeError，`run_status=failed` 无日报）
- **邮件风险指标全 N/A (a761e56)**：A 通道按扁平键读嵌套 `risk` dict ⇒ 夏普/回撤/波动率/VaR 四项全 N/A
- **ETF 价格缺口闭环 (0df0a7b, 85a915d)**：新增 `src/analysis/price_history_gate.py` 缺口闸门（按「当前持仓」过滤，已清仓标的不再误判 error）；回填 13 只在册缺口标的，缺口数 14 → 2（均 warning、无 error ⇒ 日报不再被拦死）
- **9 项 P0 整改 (126b45b)**：#140 占位哨兵 / bat 退出码 / WAL+busy_timeout / 备份条数上限 / 测试连接泄漏 / Excel `VOID_FILL` / 镜像隔离 / ETF 缺口 / 文档订正
- **neodata 指数估值采集 (781bd88)**：中文名实体解析回归致 930713/931743 长期 0 行；改为指数代码精确查询优先 + 中文名兜底，并消除 `or rows` 兜底造成的跨指数估值误标
- **折算 / 拆分伪收益治理 (2c730e6, cd67fc0, d7088f5)**：折算闸门覆盖场外基金与无 qfq 折算日；`etf_features` 位置类特征改用 qfq 合并价消除拆分日假台阶
- **契约1 判级误报 (055bbfe, 10ed187)**：场外 T+1 未披露不再判 error，按当日快照是否已覆盖区分常态 / 事故形态；评估时刻移到快照落库之后
- **数据诚实化 (14719b9, 43d06ce)**：`fund_flows` 20 行「自称完全可信的空值行」元数据诚实化；#130 资金流覆盖链三守卫 + 读取端 NULL 过滤
