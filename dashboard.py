...
def main():
    ...
    # ========== 图表行1: 净值曲线 + 收益分布 ==========
    tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8, tab9, tab10, tab11 = st.tabs(["📈 净值走势", "📊 持仓分布", "⚠️ 风险分析", "📅 收益日历", "💠 高级分析", "📡 技术信号", "📰 资讯与评估", "💡 操作建议", "🔬 自定义指标", "💰 资金动向", "🥇 黄金市场"])
    ...
    with tab6:
        ...
        if tech_df is None or tech_df.empty:
            ...
            # ---------- 信号强度雷达图 ----------
            st.markdown('<div class="tip-title" style="font-size:16px;border-bottom:none;padding:5px 0;">多空信号分布<span class="tip-arrow" style="left: 4px; top: calc(100% + 5px);"></span><span class="tip-text" style="left: 4px; top: calc(100% + 10px);">雷达图展示各维度的多空力量对比，越靠近外圈表示该维度多头信号越强。</span></div>', unsafe_allow_html=True)
            ...
            with radar_cols[0]:
                ...
                fig_radar.add_trace(go.Scatterpolar(
                    r=bull_values, theta=categories,
                    fill='toself', fillcolor='rgba(34,197,94,0.15)',
                    line_color='#22c55e', name='多头信号', marker_size=5
                ))
                fig_radar.add_trace(go.Scatterpolar(
                    r=bear_values, theta=categories,
                    fill='toself', fillcolor='rgba(239,68,68,0.15)',
                    line_color='#ef4444', name='空头信号', marker_size=5
                ))
                fig_radar.update_layout(
                    polar=dict(
                        radialaxis=dict(visible=True, range=[0, max(max(bull_values), max(bear_values), 1)],
            ...
            with radar_cols[1]:
                ...
                indicator_values = [n_bullish_ma, n_bearish_ma, n_bull_macd, n_bear_macd, n_bull_kdj, n_bear_kdj, n_overbought, n_oversold]
                bar_colors = ['#22c55e', '#ef4444', '#22c55e', '#ef4444', '#22c55e', '#ef4444', '#f59e0b', '#3b82f6']

                fig_bar = go.Figure(go.Bar(
                    x=indicator_labels, y=indicator_values,
                ...
                fig_bar.update_layout(
                    ...
                    yaxis=dict(title='持仓数量', tickfont=dict(size=10, color='#8b949e'),
            ...
            # ---------- 技术信号详情表 ----------
            st.markdown('<div class="tip-title" style="font-size:16px;border-bottom:none;padding:5px 0;">持仓技术信号详情<span class="tip-arrow" style="left: 4px; top: calc(100% + 5px);"></span><span class="tip-text" style="left: 4px; top: calc(100% + 10px);">展示每只持仓品种的完整技术指标信号。颜色标记：绿色=多头/看多，红色=空头/看空，黄色=超买/超卖预警。</span></div>', unsafe_allow_html=True)
            ...
            def _rsi_c(v):
                if v >= 80: return f'<span style="color:#ef4444;font-weight:bold">{v:.1f}</span>'
                if v >= 70: return f'<span style="color:#f59e0b;font-weight:bold">{v:.1f}</span>'
                if v <= 20: return f'<span style="color:#3b82f6;font-weight:bold">{v:.1f}</span>'
                if v <= 30: return f'<span style="color:#f59e0b">{v:.1f}</span>'
                return f'<span style="color:#c9d1d9">{v:.1f}</span>'

            def _boll_c(v):
                if v >= 80: return f'<span style="color:#ef4444;font-weight:bold">{v:.1f}%</span>'
                if v >= 60: return f'<span style="color:#22c55e">{v:.1f}%</span>'
                if v <= 20: return f'<span style="color:#3b82f6;font-weight:bold">{v:.1f}%</span>'
                if v <= 40: return f'<span style="color:#f59e0b">{v:.1f}%</span>'
                return f'<span style="color:#c9d1d9">{v:.1f}%</span>'

            def _atr_c(v):
                if v >= 3.0: return f'<span style="color:#f59e0b;font-weight:bold">{v:.2f}%</span>'
                if v >= 2.0: return f'<span style="color:#c9d1d9">{v:.2f}%</span>'
            ...
            # ---------- 布林带位置分布图 ----------
            st.markdown('<div class="tip-title" style="font-size:16px;border-bottom:none;padding:5px 0;">布林带位置分布<span class="tip-arrow" style="left: 4px; top: calc(100% + 5px);"></span><span class="tip-text" style="left: 4px; top: calc(100% + 10px);">展示所有持仓品种在布林带中的相对位置(0%=下轨, 50%=中轨, 100%=上轨)。超过100%或低于0%表示突破布林带轨道。</span></div>', unsafe_allow_html=True)
            ...
            fig_boll.update_layout(
                xaxis=dict(title='布林带位置 (%)', range=[min(-10, boll_df['bollinger_position'].min()-5),
                                                         max(110, boll_df['bollinger_position'].max()+5)],
                          tickfont=dict(size=10, color='#8b949e'), gridcolor='#21262d'),
            ...
            # ---------- RSI 分布图 ----------
            st.markdown('<div class="tip-title" style="font-size:16px;border-bottom:none;padding:5px 0;">RSI 相对强弱分布<span class="tip-arrow" style="left: 4px; top: calc(100% + 5px);"></span><span class="tip-text" style="left: 4px; top: calc(100% + 10px);">展示所有持仓品种的RSI值分布。RSI>70为超买区间(红色)，RSI<30为超卖区间(蓝色)。</span></div>', unsafe_allow_html=True)
            ...
            fig_rsi.update_layout(
                xaxis=dict(title='RSI 值', range=[0, 100],
                          tickfont=dict(size=10, color='#8b949e'), gridcolor='#21262d'),
                yaxis=dict(title='', tickfont=dict(size=10, color='#c9d1d9'), gridcolor='#21262d'),
                paper_bgcolor='#0d1117', plot_bgcolor='#0d1117',
                height=max(300, 30 * n_total),
    ...
    with tab9:
        ...
        # ----- 指标回测子Tab -----
        with tab9_sub1:
            st.markdown('<div class="tip-title" style="font-size:16px;border-bottom:none;padding:5px 0;">指标信号回测<span class="tip-arrow" style="left: 4px; top: calc(100% + 5px);"></span><span class="tip-text" style="left: 4px; top: calc(100% + 10px);">选择预置模板或自定义指标条件，对持仓ETF进行历史信号回测。</span></div>', unsafe_allow_html=True)

            try:
                from src.analysis.indicator_backtest import (
                    INDICATOR_TEMPLATES, backtest_technical_composite
                )
                ...
                if etf_code and st.button("🚀 开始回测", key="ind_run_bt", type="primary"):
                    with st.spinner("正在回测..."):
                        ...
                        try:
                            result = backtest_technical_composite(
                                conn_bt, etf_code, tmpl['formula'], lookback=lookback
                            )
                        finally:
                            conn_bt.close()
                    ...
                    if 'error' in result:
                        ...
                        st.metric("最大单次收益", f"{result['max_return_pct']:+.2f}%",
                        ...
                        if len(details) >= 3:
                            ...
                            fig_bt.update_layout(
                                xaxis=dict(title='信号日期', gridcolor='#21262d',
                                           tickfont=dict(size=9, color='#8b949e')),
                                yaxis=dict(title='收益%', gridcolor='#21262d',
                                           tickfont=dict(size=9, color='#8b949e')),
                                paper_bgcolor='#0d1117', plot_bgcolor='#0d1117',
        ...
        # ----- K线形态识别子Tab -----
        with tab9_sub2:
            st.markdown('<div class="tip-title" style="font-size:16px;border-bottom:none;padding:5px 0;">K线形态识别<span class="tip-arrow" style="left: 4px; top: calc(100% + 5px);"></span><span class="tip-text" style="left: 4px; top: calc(100% + 10px);">自动识别常见K线反转和持续形态，辅助判断市场转折点。</span></div>', unsafe_allow_html=True)

            try:
                from src.analysis.candle_patterns import (
                    detect_candle_patterns, PATTERN_NAME_MAP, PATTERN_SIGNAL
                )
                ...
                if candle_code:
                    ...
                    try:
                        snaps = pd.read_sql_query("""
                            ...
                            ORDER BY date DESC
                            LIMIT ?
                        """, conn_ck, params=[candle_code, n_candle])
                    finally:
                        conn_ck.close()
                        ...
                        if not pattern_rows.empty:
                            ...
                            with col_pc2:
                                ...
                                for _, row in pattern_rows.iterrows():
                                    ...
                                    name_str = PATTERN_NAME_MAP.get(
                                        row['pattern'].split(',')[0].strip(), ''
                                    )
                                    fig_k.add_annotation(
                                        x=row['date'], y=row['high'] * 1.005,
                ...
                if not sector_df.empty:
                    ...
                    if not latest.empty:
                        ...
                        fig_sf.update_layout(
                            ...
                            xaxis=dict(title='净流入(亿元)', gridcolor='#21262d',
                                       tickfont=dict(size=9, color='#8b949e')),
...
# 多日趋势热力图
        ...
        # ========== Tab11: 黄金市场分析 ==========
        with tab11:
            st.markdown('<div class="tip-title" style="font-size:16px;border-bottom:none;padding:5px 0;">黄金市场分析<span class="tip-arrow" style="left: 4px; top: calc(100% + 5px);"></span><span class="tip-text" style="left: 4px; top: calc(100% + 10px);">上海金交所金价走势、实时行情、SPDR持仓及中国黄金储备。</span></div>', unsafe_allow_html=True)
                        ...
                        if not plot_df.empty:
                            ...
                            if show_ma:
                                if plot_df['MA5'].notna().sum() > 0:
                                    fig_kline.add_trace(go.Scatter(x=plot_df['date'], y=plot_df['MA5'], mode='lines', name='MA5', line=dict(color='#FFD700', width=1)))
                                if plot_df['MA20'].notna().sum() > 0:
                                    fig_kline.add_trace(go.Scatter(x=plot_df['date'], y=plot_df['MA20'], mode='lines', name='MA20', line=dict(color='#FF69B4', width=1)))
                                if plot_df['MA60'].notna().sum() > 0:
                                    fig_kline.add_trace(go.Scatter(x=plot_df['date'], y=plot_df['MA60'], mode='lines', name='MA60', line=dict(color='#00CED1', width=1)))
                            ...
                            fig_kline.update_layout(
                                ...
                                font=dict(color='#ddd'), margin=dict(l=50, r=30, t=40, b=30),
                                legend=dict(orientation="h", yanchor="bottom", y=1.02)
                            )
                    ...
                    if spdr_df is not None and not spdr_df.empty:
                        ...
                        if date_col and stock_col:
                            ...
                            fig_spdr.add_trace(go.Bar(
                                ...
                                name='总库存(吨)', marker_color='#FFD700', opacity=0.7, yaxis='y'
                            ))
                            if change_col:
                        ...
                        if date_col and gold_col:
                            ...
                            if yoy_col and yoy_col in cn_gold.columns:
                                fig_cng.add_trace(go.Scatter(
                                    ...
                                    name='同比(%)', mode='lines', line=dict(color='#66BB6A', width=2),
                                    yaxis='y2'
                                ))
