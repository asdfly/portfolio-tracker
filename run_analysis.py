...
def run_stage0_otc_nav(date_str=None):
    ...
    口径与安全性：
     ...
     - 880013（招商资管智远天添利，货币型）无「单位净值」概念：akshare 两个接口都取不到
       （一个 JSEvalException，一个 14 列硬编码不匹配），改为直连东方财富 f10/lsjz 取
       每万份收益，按「净值恒 1.0、收益体现为份额增长」的货币型口径写入。
       若该直连源也失效，则跳过 + warning，绝不硬编码、绝不沿用旧值（届时按成本计价）；
